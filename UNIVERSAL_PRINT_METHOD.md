...
Метод **записывает данные начиная со стартовой ячейки**:
- Данные пишутся в стартовую ячейку и ниже
- Существующие данные в этих ячейках перезаписываются
- При вертикальном направлении (VERTICAL_DOWN_HORIZONTAL_RIGHT) строки НЕ вставляются
- Стили из шаблона-строки применяются к каждой строке данных
...
- **Порядок операций**: сначала создаются ячейки → потом копируются стили → потом заполняются данные

**Если не указано:** стили не копируются, используются стандартные.

**Пример работы:**
```
Строка-шаблон: [Стиль1] [Стиль2] [Стиль3]
Данные: value1, value2, value3

Результат:
Строка 1: [Стиль1: value1] [Стиль2: value2] [Стиль3: value3]
Строка 2: [Стиль1: value1] [Стиль2: value2] [Стиль3: value3]
...
```
